...
static esp_err_t sdmmc_init_spi_crc_optional(sdmmc_card_t* card)
{
    esp_err_t err = sdmmc_init_spi_crc(card);
    if (err == ESP_ERR_NOT_SUPPORTED) {
        ESP_LOGW(TAG, "card rejected SPI CRC enable command; continuing without data CRC");
        return ESP_OK;
    }
    return err;
}
...
esp_err_t sdmmc_card_init(const sdmmc_host_t* config, sdmmc_card_t* card)
{
    ...
    /* Enable CRC16 checks for data transfers in SPI mode if the card accepts CMD59. */
    SDMMC_INIT_STEP(is_spi, sdmmc_init_spi_crc_optional);
